...
#define CAPTURE_PROPERTY_SHARPNESS		CAPTURE_PROPERTY( 'Q', 'S', 'R', 'P' )
	/* R/W		int32_t			Sharpness, -128 (blurred image) to 127 (sharpest image). */
...
#define CAPTURE_DEI_WEAVE_FLAG_BOTTOM_TOP	(1 << 0)
	/*					If set, always weave bottom + top, possibly dropping an unexpected first top field;
	 *					if not set, always weave top + bottom, possibly dropping an unexpected first bottom field. */
...
#define CAPTURE_PROPERTY_NO_SIGNAL_MODE		CAPTURE_PROPERTY( 'Q', 'N', 'O', 'S' )
	/* R/W		uint32_t		No signal mode, set default action if 'no carrier' or 'no signal' state is detected. */

/*
    CAPTURE_NO_SIGNAL_DEFAULT_MODE - Default behavior, defined by driver or hardware (unknown).
    CAPTURE_NO_SIGNAL_NOISE_MODE   - Show color or black-and-white noise (snow) on the screen if there is no input video source.
                                     Common mode for hardware which do not supports detection of video source signal.
    CAPTURE_NO_SIGNAL_BLACK_MODE   - Fill all frames with black (or close to black) color if no video source signal state is detected.
    CAPTURE_NO_SIGNAL_BLUE_MODE    - Fill all frames with blue (or close to blue) color if no video source signal state is detected.
    CAPTURE_NO_SIGNAL_TABLE_MODE   - Use a hardware or software pre-defined picture (usually with text "NO SIGNAL") if no video
...
#define CAPTURE_PROPERTY_VERBOSITY_FILE		CAPTURE_PROPERTY('Q','V','B','F')
	/* R/W		FILE*			Log messages are written to a file, default is stderr.
	 *					A NULL pointer disables logging. */
#define CAPTURE_PROPERTY_LOGMSG_PREFIX		CAPTURE_PROPERTY('Q','L','M','P')
	/* R/W		char*			Prefix log messages with a user defined string. */
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/lib/capture/public/vcapture/capture.h $ $Rev: 761333 $")
#endif
