...
start-wifi ()
{
	...
	if [ ! -e "$FIRMWARE_PATH/firmware_ti18xx_mcp8.bin" ]; then
		ln -Ps /lib/firmware/ti18xx/firmware_ti18xx_mcp8.bin $FIRMWARE_PATH/firmware_ti18xx_mcp8.bin
	fi
	for file in tiwlan.bin firmware_ti18xx_mcp8.bin; do
    		if [ ! -e "$FIRMWARE_PATH$file" ]; then
        ...
        mount -Tio-pkt -o firmdir=$FIRMWARE_PATH devnp-ti18xx_omap5.so
