 ...
 *
 * @author lgreenway
 * $Id: pbap.js 4726 2012-10-24 15:55:42Z lgreenway@qnx.com $
 */
...
module.exports = {
	...
	init: function() {
		try {
			...
			_statusPPS.open("/pps/services/bluetooth/phonebook/status", JNEXT.PPS_RDONLY);
		} catch(ex) {
		...
		try {
			...
			_commandPPS.init();
			_commandPPS.open("/pps/services/bluetooth/phonebook/control", JNEXT.PPS_WRONLY);
		} catch(ex) {
	...
	getState: function() {
		...
		if(_statusPPS.ppsObj && _statusPPS.ppsObj.state) {
			state = pbapStateToServiceState(_statusPPS.ppsObj.state);
		} else {
	...
	getStatus: function() {
		var state = null;
		if(_statusPPS.ppsObj && _statusPPS.ppsObj.status) {
			state = pbapStatusToServiceStatus(_statusPPS.ppsObj.status);
		} else {
