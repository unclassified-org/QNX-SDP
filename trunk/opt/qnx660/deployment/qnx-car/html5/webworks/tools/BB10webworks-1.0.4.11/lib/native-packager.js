...
function generateTabletXMLFile(session, config) {
	...
	//adding category to object representing XML structure
	if (config.category) {
		xmlObject.category = config.category;
	}
...
function execNativePackager(session, callback) {
    ...
    logger.info("Calling Script: [" + script + "]...");
    nativePkgr = childProcess.spawn(path.normalize(script), ["@options"], {
        "cwd": cwd,
        "env": process.env
    });
