 ...
 *}</code></pre></dd></dl>
 * 
 * @module qnx_xyz_message
 */

/* @author lgreenway, mlytvynyuk
 * $Id: client.js 4377 2012-09-28 22:34:57Z lgreenway@qnx.com $
 */
...
function Message(properties) {
	 ...
	 */
	this.save = function () {
		this.uid = window.webworks.execSync(_ID, 'save', { contact:this });
	};
	...
	this.remove = function () {
		window.webworks.execSync(_ID, 'remove', { contact:this });
	};
	...
	this.move = function (folder) {
		window.webworks.execSync(_ID, 'move', { contact:this, folder:folder });
	};
	 ...
	 */
	this.send = function () {
		window.webworks.execSync(_ID, 'send', { contact:this });
	};
};
...
module.exports = {
	...
	getAccounts: function(messageType) {
		...
		return window.webworks.execSync(_ID, 'getAccounts', args);
	},
	...
	find:function (filter, orderBy, isAscending, limit, offset) {
		...
		var results = window.webworks.execSync(_ID, 'find', args),
			messages = [];

		// Build Contact instances for each result
		for (var result in results) {
			messages.push(new Message(results[result]));
		}
	...
	getMessage:function (accountId, handle) {
		window.webworks.execAsync(_ID, 'getMessage', { accountId: accountId, handle:handle });
	},
