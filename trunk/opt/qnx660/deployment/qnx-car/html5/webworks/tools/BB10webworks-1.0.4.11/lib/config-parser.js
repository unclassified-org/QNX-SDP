...
/**
 * Look for category in config,
 * if present extract the name and include in config object
 * */
function processCategoryData(data, widgetConfig) {
	var category = data["rim:category"];
	if(category) {
		var attribs = category["@"];
		widgetConfig.category = sanitize(attribs.name).trim();
	}
}
...
function processResult(data, session, extManager) {
	...
	processCategoryData(data, widgetConfig);
