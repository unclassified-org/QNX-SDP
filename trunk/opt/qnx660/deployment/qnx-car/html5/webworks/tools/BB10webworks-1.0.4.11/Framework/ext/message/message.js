 ...
 *
 * @author mlytvynyuk
 * $Id: message.js 4784 2012-10-29 21:17:38Z lgreenway@qnx.com $
 */
...
/**
 * Event handler will be invoked during onChange on PPS object.
...
function queryMessages(whereClause, orderBy, isAscending, limit, offset) {
	...
	var result = _sqlite.resultToArray(_db.query(sql));
...
module.exports = {
	...
	init:function () {
		try {
			...
			_statusPPS.open("/pps/services/bluetooth/messages/status", JNEXT.PPS_RDONLY);
		} catch (ex) {
		...
		try {
			...
			_controlPPS.init();
			_controlPPS.open("/pps/services/bluetooth/messages/control", JNEXT.PPS_WRONLY);
		} catch (ex) {
		...
		try {
			...
			_notificationPPS = _pps.createObject();
			_notificationPPS.init();
			_notificationPPS.onChange = onNotificationEvent;
			_notificationPPS.open("/pps/services/bluetooth/messages/notification", JNEXT.PPS_RDONLY);
		} catch (ex) {
	...
	getAccounts: function(messageType) {
		...
		// Get the results
		var results = _sqlite.resultToArray(_db.query(sql));
	...
	getMessage:function (accountId, handle) {
		...
		if (message) {
			...
			if (_statusPPS.ppsObj.state != ServiceStatus.STATUS_PROCESSING) {
				// redefine trigger
	...
	getState:function () {
		var state = null;
		if (_statusPPS.ppsObj && _statusPPS.ppsObj.state) {
			state = mapStateToServiceState(_statusPPS.ppsObj.state);
		} else {
