 ...
 */
 ...
 //enum {
 	...
 	*/
 	self.init = function() {
        ...
        if (!JNEXT.require('jscreen')) {
	    	console.error("Unable to load jscreen.");
            return false;
        }
 		...
 		if ( !jscreenObjCreated )  {
 			if ( (self.m_strObjId = JNEXT.createObject( "jscreen.JScreen" )) == "" )  {
 				self.connecting = false;
 				console.error("error initializing jscreen.so - check if I am allowed to be a screen window manager");
 				return false;
 			}
 ...
 module.exports = {createObject : function () {return JNEXT.JScreen}};
