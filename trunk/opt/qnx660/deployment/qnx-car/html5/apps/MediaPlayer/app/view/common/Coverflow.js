 ...
 *
 * $Id: Coverflow.js 6123 2013-04-29 19:03:31Z lgreenway@qnx.com $
 */
Ext.define('MediaPlayer.view.common.Coverflow',{
	 ...
	 * dragged. Prevents display updates from occurring when setActiveItem
	 * or setSelectedIndex is called.
	 */
	dragging: false,
	...
	*/
	...
	resetScrollerTransitionDuration: function(ms) {
		...
		this.getScrollerContainer().dom.style.webkitTransitionDuration = (typeof(ms) == 'number' ? ms + 'ms' : '500ms');
	},
	...
	/**
	 * setSelectedIndex alias for interface compatibility with other
	 * coverflow component.
	 * @param idx {Number} The item index.
	 */
	setActiveItem: function() {
		this.callParent(arguments);

		this.setSelectedIndex.apply(this, arguments);
	},
	...
	onPainted: function() {
		...
		// Initialize the scroller transition style
		this.resetScrollerTransitionDuration();
		
		//Sencha 2.1 Hack
		//Force the Coverflow to Select Item 0
	...
	onTouchMove: function(e){
		...
		if(this.dragging) {
			...
			this.lastDragDeltas.push({ delta: delta, time: (new Date()).getTime() });
	...
	refresh: function() {
		if(this.painted) {
			...
			if(this.getStore().getCount() > 0) {
				// Remove the empty class if it's present
	...
	moveCoverflowItem: function(toFront) {
		if(toFront) {
			...
			item.setSong(this.getStore().getAt(item.getIdx()));
	...
	reindexCoverflowItems: function(newIndex) {
		if(this.getItems().length > 0 && (newIndex >= 0 && newIndex <= this.getStore().getCount()-1)) {
			...
			if(newIndex < this.getAt(0).getIdx() || newIndex > this.getAt(this.getItems().length-1).getIdx()) {
				...
				var firstIndex = 0;
				if(newIndex > this.getStore().getCount() - (this.getMaxDisplayedItems() / 2)) {
					firstIndex = Math.max(0, this.getStore().getCount() - this.getMaxDisplayedItems());
				} else if(newIndex < Math.min(this.getMaxDisplayedItems(), this.getStore().getCount())) {
					firstIndex = 0;
				} else {
					firstIndex = newIndex - (this.getMaxDisplayedItems() / 2);
				}
				...
				while(this.getAt(0).getIdx() < newIndex - (this.getMaxDisplayedItems() / 2)
						&& this.getAt(this.getItems().length-1).getIdx() < this.getStore().getCount()-1) {
					this.moveCoverflowItem(false);
				}

				while(this.getAt(this.getItems().length-1).getIdx() > newIndex + (this.getMaxDisplayedItems() / 2)
						&& this.getAt(0).getIdx() > 0) {
